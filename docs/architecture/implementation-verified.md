...
## attribute-types.service.ts

Per-portfolio CRUD for `attribute_type` (PP vendor table). Builtin IDs
(`logo`, `aum`, `ter`, `acquisitionFee`, `managementFee`) are protected at the
service layer (`BUILTIN_TYPE_PROTECTED`). Cascade delete removes all
dependent `security_attr` rows in a single SQLite transaction.
