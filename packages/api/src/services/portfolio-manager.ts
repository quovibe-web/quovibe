...
export type CreatePortfolioInput =
  ...
  | { source: 'import-quovibe-db'; uploadedDbPath: string; name?: string };

export interface CreatePortfolioResult {
  entry: PortfolioEntry;
  summary?: ImportSummary;        // populated for import-pp-xml + import-quovibe-db
  alreadyExisted?: boolean;       // true for idempotent demo re-creation
}

export class PortfolioManagerError extends Error {
  // BUG-PRE14-04: `conflictingName` carries the registry entry that collided
  // on DUPLICATE_NAME so the route layer can echo the actual switcher label
  // back to the client. Without it the client falls back to the user-supplied
  // filename stem, which need not match what is stored in vf_portfolio_meta.
  constructor(
...
function assertUniquePortfolioName(name: string, selfId?: string): void {
  ...
  if (target === DEMO_PORTFOLIO_NAME_LC) {
    ...
    if (!selfAlreadyHasName) {
      const demoEntry = entries.find(p => p.kind === 'demo');
      throw new PortfolioManagerError(
        'DUPLICATE_NAME',
        undefined,
        demoEntry?.name ?? 'Demo Portfolio',
      );
    }
  }
  ...
  if (clash) throw new PortfolioManagerError('DUPLICATE_NAME', undefined, clash.name);
}
...
async function createPortfolioImpl(input: CreatePortfolioInput): Promise<CreatePortfolioResult> {
  ...
  if (input.source === 'import-quovibe-db') {
    return createImportedQuovibeDbImpl(input.uploadedDbPath, input.name);
  }
...
function createImportedPpxmlImpl(name: string, ppxmlTempDbPath: string): CreatePortfolioResult {
  ...
  try {
    ...
    seedDefaultDashboard(db);
    summary = computeImportSummary(db);
  } finally {
    db.close();
  }
  finalizeRegistry(entry);
...
function createImportedQuovibeDbImpl(
  ...
  // Registry-name guard runs symmetrically with PP-XML import (see
  // .claude/rules/portfolio-creation.md "Duplicate-name invariant"). The
  // override does not rewrite inner vf_portfolio_meta — registry name is the
  // user-facing label, inner meta is the backup's archival self-description.
  // When an override is supplied, run the guard FIRST to skip the
  // vf_portfolio_meta read on the override-collision path.
  const trimmedOverride = nameOverride?.trim() ?? '';
  if (trimmedOverride) assertUniquePortfolioName(trimmedOverride);
  ...
  const finalName = trimmedOverride || sourceName;
  if (!trimmedOverride) assertUniquePortfolioName(finalName);
  const entry: PortfolioEntry = {
    id, name: finalName, kind: 'real', source: 'import-quovibe-db',
    createdAt,
  ...
  // Compute ImportSummary in the same open to avoid a second round-trip.
  let summary: ImportSummary;
  const db = new Database(filePath);
  try {
    applyBootstrap(db);
...
function computeImportSummary(db: Database.Database): ImportSummary {
  return {
    accounts:     db.prepare("SELECT COUNT(*) FROM account WHERE type='account'").pluck().get() as number,
    securities:   db.prepare('SELECT COUNT(*) FROM security').pluck().get() as number,
    transactions: db.prepare(
      "SELECT COUNT(*) FROM xact WHERE NOT (type IN ('BUY','SELL') AND shares=0)",
    ).pluck().get() as number,
  };
}
