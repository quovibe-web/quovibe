...
describe('dashboard.service.columns — auto-sentinel round-trip', () => {
  it("create with columns='auto' stores sentinel int 1 and reads back as 'auto'", () => {
    const sqlite = freshSqlite();
    const created = createDashboard(sqlite, { name: 'A', widgets: [], columns: 'auto' });
    expect(created.columns).toBe('auto');

    const raw = sqlite.prepare('SELECT columns FROM vf_dashboard WHERE id = ?')
      .get(created.id) as { columns: number };
    expect(raw.columns).toBe(1);

    const fetched = getDashboard(sqlite, created.id);
    expect(fetched?.columns).toBe('auto');
    sqlite.close();
  });

  it('explicit numeric columns round-trip untouched', () => {
    const sqlite = freshSqlite();
    for (const c of [2, 3, 4, 5] as const) {
      const created = createDashboard(sqlite, { name: `D${c}`, widgets: [], columns: c });
      expect(created.columns).toBe(c);
      const raw = sqlite.prepare('SELECT columns FROM vf_dashboard WHERE id = ?')
        .get(created.id) as { columns: number };
      expect(raw.columns).toBe(c);
    }
    sqlite.close();
  });

  it("updateDashboard switches stored sentinel from a numeric value to 'auto' and back", () => {
    const sqlite = freshSqlite();
    const created = createDashboard(sqlite, { name: 'A', widgets: [], columns: 3 });

    const toAuto = updateDashboard(sqlite, created.id, { columns: 'auto' });
    expect(toAuto?.columns).toBe('auto');
    let raw = sqlite.prepare('SELECT columns FROM vf_dashboard WHERE id = ?')
      .get(created.id) as { columns: number };
    expect(raw.columns).toBe(1);

    const toFour = updateDashboard(sqlite, created.id, { columns: 4 });
    expect(toFour?.columns).toBe(4);
    raw = sqlite.prepare('SELECT columns FROM vf_dashboard WHERE id = ?')
      .get(created.id) as { columns: number };
    expect(raw.columns).toBe(4);
    sqlite.close();
  });
});
