...
import type { CreatePortfolioInput, CreatePortfolioResult, FreshPortfolioInput } from '../portfolio-manager';
import { buildPpxmlTempDb, buildQuovibeBackupDb } from './_helpers/build-backup-db';
...
describe('portfolio-manager', () => {
  ...
  describe('createImportedPpxmlImpl — summary', () => {
    it('returns ImportSummary with COUNTs from the freshly-bootstrapped DB', async () => {
      const tempDbPath = buildPpxmlTempDb({
        depositAccounts: 2,
        securities: 5,
        transactions: 11,
      });
      ...
      const r = await createPortfolio({
        source: 'import-pp-xml',
        name: `Summary fixture ${Date.now()}`,
        ppxmlTempDbPath: tempDbPath,
      });

      expect(r.summary).toEqual({ accounts: 2, securities: 5, transactions: 11 });
    });
  });

  describe('createImportedQuovibeDbImpl — summary', () => {
    it('returns ImportSummary with COUNTs from the restored DB', async () => {
      // Build a "source" .db (pretend backup) with known counts, then restore.
      ...
      const r = await createPortfolio({
        ...
        uploadedDbPath: sourcePath,
        name: `Summary fixture ${Date.now()}`,
      });

      expect(r.summary).toEqual({ accounts: 1, securities: 3, transactions: 7 });
    });
  });
});
