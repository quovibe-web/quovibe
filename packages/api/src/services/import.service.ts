...
// BUG-PRE14-02: Stderr substring patterns that identify ppxml2db failures
// caused by malformed-but-superficially-valid input XML rather than a
// genuine subprocess / system fault. The classifier is conservative —
// patterns must appear verbatim in a real Python traceback. We never echo
// the matched substring; the bool result alone changes the wire status
// from 500 CONVERSION_FAILED to 400 INVALID_FORMAT.
const USER_XML_ERROR_PATTERNS: readonly RegExp[] = [
  /\bAssertionError\b/,
  /\bKeyError\b/,
  /\bAttributeError\b/,
  /\bTypeError\b/,
  /\bValueError\b/,
  /\bIndexError\b/,
  /\bxml\.etree\.ElementTree\.ParseError\b/,
  /\bElementTree\.ParseError\b/,
  /\bParseError\b/,
  /\bSyntaxError\b/,
  /\bExpatError\b/,
];

/** True when ppxml2db's stderr looks like a user-input bug, not a subprocess fault. */
export function looksLikeUserXmlBug(stderr: string): boolean {
  return USER_XML_ERROR_PATTERNS.some(re => re.test(stderr));
}
 ...
 * 4. Return { tempDbPath }
 *
 * Bootstrap DDL + entity counts run later in createImportedPpxmlImpl, on
 * the destination DB after atomicCopy — see portfolio-manager.ts.
 *
...
export async function runImport(xmlPath: string): Promise<ImportResult> {
  ...
  try {
    ...
    try {
      ...
      // BUG-PRE14-02: classify server-side. Schema-valid-but-ppxml2db-
      // incompatible XML produces a Python exception (AssertionError /
      // KeyError / ParseError / etc.) — that is a 400 user-input bug.
      // Subprocess crashes (SIGKILL, ENOENT, timeout, OSError) stay
      // 500. The classifier is a stderr-substring scan; we never echo
      // any of it back on the wire.
