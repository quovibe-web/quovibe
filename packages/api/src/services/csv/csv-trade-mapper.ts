...
export function mapTradeRows(
  ...
  for (const row of rows) {
    ...
    // Per-row resolved UUIDs override the file-level default for each
    // routing slot. The service layer pre-resolves the four CSV name cells
    // via `resolveAccountNames` and attaches the result to `row.accountUuids`;
    // empty/missing fields fall back to ctx defaults per the per-type matrix.
    const secAccount = row.accountUuids?.securitiesAccount ?? ctx.portfolioId;
    const cashAccount = row.accountUuids?.account ?? ctx.depositAccountId;
    ...
    if (GROUP_A_DUAL_ENTRY.has(txType)) {
      ...
      // FOREX unit for cross-currency DIVIDEND. Mirrors the BUY/SELL block
      // above and the equivalent transaction.service.ts > buildUnits DIVIDEND
      // emission so JSON and CSV ingestion paths persist byte-identical
      // shapes for the engine. Other Group B types (DEPOSIT, REMOVAL,
      // INTEREST, …) carry no security and never trip the cross-currency
      // gate — gating on `txType === DIVIDEND` keeps this strictly opt-in.
      if (
        ...
        if (secCcy && secCcy !== currency) {
          ...
          units.push(makeUnit(xactId, 'FOREX', grossDepHecto, currency, {
            amount: grossSecHecto,
            currency: secCcy,
            rate: String(row.fxRate),
          }));
      ...
      // Per-row routing: source = accountUuids.securitiesAccount (or ctx
      // default); destination = accountUuids.offsetSecuritiesAccount.
      // The MISSING_ACCOUNT gate in collectAccountResolveErrors ensures this
      // is populated before the mapper runs; skip silently as defense-in-depth.
      const destSecAccount = row.accountUuids?.offsetSecuritiesAccount;
      if (!destSecAccount) continue;
      ...
      // Per-row routing: source = accountUuids.account; destination =
      // accountUuids.offsetAccount. The MISSING_ACCOUNT gate in
      // collectAccountResolveErrors ensures both are populated before the
      // mapper runs; skip silently as defense-in-depth.
      ...
      if (row.fxRate != null) {
        const dstCcy = ctx.accountCurrencyMap?.get(destCashAccount);
...
function resolveCrossCurrency(
  ...
  // BUY / SELL / DIVIDEND share the same shape: the security's currency vs
  // the deposit (portfolio) currency. DIVIDEND lands as Group B (cash-only
  // routing to the deposit) but its security-vs-cash currency mismatch is
  // exactly the same pair the FX gate cares about, so it shares the BUY/SELL
  // resolver. Pending-new securities (preview time, before the new security
  // exists) are absent from `securityCurrencyMap` and skip the gate; the
  // execute pass catches them after the security is persisted.
  if (
  ...
  if (txType === TransactionType.TRANSFER_BETWEEN_ACCOUNTS) {
    const destCashAccount = row.accountUuids?.offsetAccount;
    if (!destCashAccount) return null;
    const dstCcy = ctx.accountCurrencyMap?.get(destCashAccount) ?? null;
    return {
