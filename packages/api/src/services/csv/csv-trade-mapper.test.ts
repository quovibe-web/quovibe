...
describe('mapTradeRows', () => {
  ...
  describe('Group D — SECURITY_TRANSFER (dual portfolio-transfer)', () => {
    it('produces 2 xact rows + 1 portfolio-transfer cross-entry', () => {
      const rows: NormalizedTradeRow[] = [{
        ...
        fees: 10,
        taxes: 0,
        accountUuids: { offsetSecuritiesAccount: 'port-dest' },
      }];
  ...
  describe('Group E — TRANSFER_BETWEEN_ACCOUNTS (dual account-transfer)', () => {
    it('produces 2 xact rows + 1 account-transfer cross-entry', () => {
      const rows: NormalizedTradeRow[] = [{
        ...
        fees: 5,
        taxes: 0,
        accountUuids: { offsetAccount: 'dep-dest' },
      }];
  ...
  describe('Cross-currency gate', () => {
    ...
    it('Cross-ccy DIVIDEND without fxRate when security and deposit ccy differ → FX_RATE_REQUIRED', () => {
      // DIVIDEND joined CROSS_CURRENCY_FX_TYPES once it became clear that a
      // USD-denominated dividend paid into an EUR deposit needs an explicit
      // fxRate to persist the FOREX leg deterministically. Skipping the gate
      // would leave `units:[]` on the transaction and silently corrupt
    ...
    it('Cross-ccy DIVIDEND with fxRate emits a FOREX unit on the cash row', () => {
      // qvFxRate ≈ 0.923 (1 USD = 1.0837 EUR per the BUY example) — same
      // arithmetic as the BUY-FOREX assertion above.
      const qvFxRate = 1 / 1.0837;
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-03-15',
        type: TransactionType.DIVIDEND,
        securityName: 'Apple Inc',
        amount: 50,           // EUR (deposit ccy)
        grossAmount: 54.185,  // USD (security ccy) ≈ 50 × 1.0837
        fxRate: qvFxRate,
      }];
      ...
      const forex = result.units.find((u) => u.type === 'FOREX');
      expect(forex).toBeDefined();
      expect(forex!.xact).toBe(result.transactions[0].id); // cash-only row
      expect(forex!.currency).toBe('EUR');                 // deposit ccy
      expect(forex!.amount).toBe(5000);                    // 50 × 100, deposit hecto
      expect(forex!.forex_currency).toBe('USD');           // security ccy
      expect(forex!.forex_amount).toBe(5419);              // 54.185 × 100, security hecto (rounded)
      expect(forex!.exchangeRate).toBe(String(qvFxRate));
    });

    it('Same-currency DIVIDEND emits no FOREX unit (security ccy === deposit ccy)', () => {
      // Sanity that the DIVIDEND branch is gated on a real currency
      // mismatch and does not over-emit on plain-EUR portfolios.
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-03-15',
        type: TransactionType.DIVIDEND,
      ...
      expect(result.errors).toHaveLength(0);
      expect(result.transactions).toHaveLength(1);
      expect(result.units.find((u) => u.type === 'FOREX')).toBeUndefined();
    });
  ...
  describe('per-row account routing', () => {
    it('BUY uses per-row accountUuids.securitiesAccount + accountUuids.account', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-01-15',
        type: TransactionType.BUY,
        ...
        shares: 10,
        amount: 1500,
        accountUuids: { securitiesAccount: 'port-X', account: 'dep-Y' },
      }];
      ...
      // Securities-side row uses per-row override
      expect(result.transactions[0].accountId).toBe('port-X');
      // Cash-side row uses per-row override
      expect(result.transactions[1].accountId).toBe('dep-Y');

      // Cross-entry mirrors per-row UUIDs
      const ce = result.crossEntries[0];
      expect(ce.fromAcc).toBe('port-X');
      expect(ce.toAcc).toBe('dep-Y');
    });

    it('BUY with empty accountUuids falls back to file-level defaults', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-01-15',
        type: TransactionType.BUY,
      ...
      // Falls back to ctx.portfolioId / ctx.depositAccountId
      expect(result.transactions[0].accountId).toBe('port-1');
      expect(result.transactions[1].accountId).toBe('dep-1');
      expect(result.crossEntries[0].fromAcc).toBe('port-1');
      expect(result.crossEntries[0].toAcc).toBe('dep-1');
    });

    it('TRANSFER_BETWEEN_ACCOUNTS uses accountUuids.account (src) + .offsetAccount (dest)', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-06-01',
        type: TransactionType.TRANSFER_BETWEEN_ACCOUNTS,
        securityName: '',
        amount: 3000,
        accountUuids: { account: 'dep-src', offsetAccount: 'dep-dst' },
      }];
      ...
      // Source row routed to per-row source deposit
      expect(result.transactions[0].accountId).toBe('dep-src');
      expect(result.transactions[0].type).toBe('TRANSFER_OUT');
      // Destination row routed to per-row dest deposit
      expect(result.transactions[1].accountId).toBe('dep-dst');
      expect(result.transactions[1].type).toBe('TRANSFER_IN');

      const ce = result.crossEntries[0];
      expect(ce.fromAcc).toBe('dep-src');
      expect(ce.toAcc).toBe('dep-dst');
    });

    it('SECURITY_TRANSFER uses accountUuids.securitiesAccount (src) + .offsetSecuritiesAccount (dest)', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-05-01',
        type: TransactionType.SECURITY_TRANSFER,
        ...
        amount: 2000,
        accountUuids: {
          securitiesAccount: 'port-src',
          offsetSecuritiesAccount: 'port-dst',
        },
      }];
      ...
      expect(result.transactions[0].accountId).toBe('port-src');
      expect(result.transactions[0].type).toBe('TRANSFER_OUT');
      expect(result.transactions[1].accountId).toBe('port-dst');
      expect(result.transactions[1].type).toBe('TRANSFER_IN');

      const ce = result.crossEntries[0];
      expect(ce.fromAcc).toBe('port-src');
      expect(ce.toAcc).toBe('port-dst');
    });

    it('DELIVERY_INBOUND uses per-row accountUuids.securitiesAccount, overriding default', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-04-01',
        type: TransactionType.DELIVERY_INBOUND,
        ...
        shares: 20,
        amount: 3000,
        accountUuids: { securitiesAccount: 'port-X' },
      }];

      const result = mapTradeRows(rows, ctx);
      expect(result.errors).toHaveLength(0);
      expect(result.transactions).toHaveLength(1);
      // Per-row override wins over ctx.portfolioId
      expect(result.transactions[0].accountId).toBe('port-X');
      expect(result.transactions[0].acctype).toBe('portfolio');
    });

    it('DEPOSIT (cash-only) uses per-row accountUuids.account, overriding default deposit', () => {
      const rows: NormalizedTradeRow[] = [{
        rowNumber: 1,
        date: '2024-01-10',
        type: TransactionType.DEPOSIT,
        securityName: '',
        amount: 5000,
        accountUuids: { account: 'dep-Y' },
      }];

      const result = mapTradeRows(rows, ctx);
      expect(result.errors).toHaveLength(0);
      expect(result.transactions).toHaveLength(1);
      expect(result.transactions[0].accountId).toBe('dep-Y');
    });
  });
});
