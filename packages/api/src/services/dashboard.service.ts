...
// vf_dashboard.columns is INTEGER NOT NULL. The wire/API surface exposes
// 'auto' | 2 | 3 | 4 | 5 (UI dropdown options). The sentinel int 1 — never
// offered in the dropdown — is the on-disk encoding for 'auto'.
const AUTO_COLUMNS_SENTINEL = 1;

function encodeColumns(value: DashboardColumns): number {
  return value === 'auto' ? AUTO_COLUMNS_SENTINEL : value;
}
function decodeColumns(stored: number): DashboardColumns {
  return stored === AUTO_COLUMNS_SENTINEL ? 'auto' : (stored as Exclude<DashboardColumns, 'auto'>);
}
...
function rowToItem(row: Record<string, unknown>): DashboardItem {
  ...
  return {
    ...
    widgets,
    columns: decodeColumns(row.columns as number),
...
export function createDashboard(
  ...
  input: { name: string; widgets: unknown[]; columns: DashboardColumns },
): DashboardItem {
  ...
  sqlite.prepare(
    `INSERT INTO vf_dashboard
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
  ).run(id, input.name, position, JSON.stringify(input.widgets), CURRENT_VERSION, encodeColumns(input.columns), now, now);
...
export function updateDashboard(
  sqlite: BetterSqlite3.Database,
  id: string,
  input: { name?: string; widgets?: unknown[]; columns?: DashboardColumns; position?: number },
): DashboardItem | null {
  ...
  const txn = sqlite.transaction(() => {
    ...
    sqlite.prepare(
      ...
      CURRENT_VERSION,
      encodeColumns(input.columns ?? existing.columns),
      newPos,
      now,
      id,
    );
