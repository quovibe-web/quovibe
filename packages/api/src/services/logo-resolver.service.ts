...
async function fetchByDomain(domain: string): Promise<string> {
  // Google's s2/favicons returns the issuer's real branded favicon for known
  // domains (often only a few hundred bytes — small PNG marks like the
  // iShares "i" or Vanguard "V"). A previous byte-size threshold rejected
  // these as "placeholders"; that heuristic was empirically wrong and is
  // gone. The downside: when a domain has no favicon at all, Google's
  // default globe leaks through. Acceptable — user can override manually.
...
export async function resolveLogo(input: LogoResolveRequest): Promise<string> {
  ...
  try {
    ...
    // Yahoo /search sometimes returns stale EQUITY/UNKNOWN for ETFs/funds;
    // quoteSummary classifies more reliably. Caller hint is fallback only
    // (CRYPTO short-circuit above already consumed the hint when set).
    const fromSummary = normalizeInstrumentType(summary.quoteType?.quoteType ?? '');
    const detected = fromSummary === InstrumentType.UNKNOWN
