...
import { createPortfolioSchema, patchPortfolioSchema } from '@quovibe/shared';
import {
...
import { unlinkDbFile } from '../lib/atomic-fs';
...
const postCreate: RequestHandler = async (req, res) => {
  try {
    if (req.is('multipart/form-data')) {
      ...
      // Optional rename override; empty/whitespace falls through to the
      // source's vf_portfolio_meta name inside createImportedQuovibeDbImpl.
      ...
      try {
        ...
        const result = await createPortfolio({
          ...
          uploadedDbPath: file.path,               // name read from vf_portfolio_meta unless overridden
          name: bodyName || undefined,
        });
        res.status(201).json(result);
        return;
      } finally {
        // createImportedQuovibeDbImpl may throw DUPLICATE_NAME before
        // atomicCopy runs, so cleanup must cover the rejection path too. Use
        // unlinkDbFile so the SQLite -wal/-shm sidecars created by the
        // validateQuovibeDbFile readonly open are also removed.
        unlinkDbFile(file.path);
      }
    }
