...
describe('Gate 4: portfolio .db roundtrip', () => {
  it('export → import produces identical summaries', async () => {
    ...
    // BUG-173: import-quovibe-db now runs the registry-name guard, so the
    // re-imported portfolio must use a non-colliding override name. The
    // inner vf_portfolio_meta is not rewritten by the override path, so
    // readMeta(second.id) still reads 'Roundtrip' below.
    const { entry: second } = await createPortfolio({
      source: 'import-quovibe-db',
      uploadedDbPath: out.filePath,
      name: 'Roundtrip Restored',
    });
