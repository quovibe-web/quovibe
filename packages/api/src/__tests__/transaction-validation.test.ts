...
describe('transaction write validation', () => {
  ...
  // ────────────────────────────────────────────────────────────────────────
  // Cross-currency DIVIDEND must require fxRate just like BUY/SELL.
  //
  // Pre-fix posture: a USD-denominated security paying a dividend into an
  // EUR deposit slipped through `enforceCrossCurrencyFxRate` (which only
  // branched on BUY/SELL/TRANSFER_BETWEEN_ACCOUNTS) and persisted with no
  // FOREX `xact_unit`, silently corrupting downstream cash conversions.
  // The route gate now resolves the cash side either directly (deposit
  // accountId) or via `referenceAccount` (portfolio accountId) — same shape
  // as the BUY/SELL branch, mirroring the matrix in
  // `.claude/rules/double-entry.md`.
  // ────────────────────────────────────────────────────────────────────────
  describe('DIVIDEND cross-currency FX gate', () => {
    interface DividendFixture {
      app: ReturnType<typeof createApp>;
      pid: string;
      accts: SeededAccounts;
      usdSecId: string;
    }

    async function makeDividendFixture(): Promise<DividendFixture> {
      const ctx = await makePortfolio();
      const usdSecId = randomUUID();
      const h = acquirePortfolioDb(ctx.pid);
      try {
        h.sqlite.prepare(
          `INSERT INTO security (_id, uuid, name, currency, updatedAt)
           VALUES (340, ?, 'USD-DIV-SEC', 'USD', '2026-01-01T00:00:00Z')`,
        ).run(usdSecId);
      } finally {
        releasePortfolioDb(ctx.pid);
      }
      return { ...ctx, usdSecId };
    }

    it('cross-currency DIVIDEND on a deposit accountId without fxRate returns 400 FX_RATE_REQUIRED', async () => {
      const { app, pid, accts, usdSecId } = await makeDividendFixture();

      // EUR deposit accountId, USD security — currency pair differs, no fxRate.
      const r = await request(app).post(`/api/p/${pid}/transactions`).send({
        date: '2026-01-01',
        type: 'DIVIDEND',
        amount: 50,
    ...
    it('cross-currency DIVIDEND on a portfolio accountId (auto-routed to referenceAccount) without fxRate returns 400 FX_RATE_REQUIRED', async () => {
      const { app, pid, accts, usdSecId } = await makeDividendFixture();

      // portfolioA has referenceAccount = depositA (EUR). DIVIDEND with USD
      // security must therefore demand fxRate.
      const r = await request(app).post(`/api/p/${pid}/transactions`).send({
        date: '2026-01-01',
        type: 'DIVIDEND',
        amount: 50,
    ...
    it('cross-currency DIVIDEND with fxRate succeeds and persists a FOREX xact_unit', async () => {
      const { app, pid, accts, usdSecId } = await makeDividendFixture();

      const qvFxRate = 1.10; // 1 EUR = 1.10 USD
      const r = await request(app).post(`/api/p/${pid}/transactions`).send({
        date: '2026-01-01',
        type: 'DIVIDEND',
      ...
      // GET the transaction back and assert the FOREX unit was emitted.
      const xid = r.body.uuid as string;
      const det = await request(app).get(`/api/p/${pid}/transactions/${xid}`);
      expect(det.status).toBe(200);
      const units = det.body.units as Array<{
        type: string;
        amount: number;
      ...
      expect(forex, `expected FOREX unit, got ${JSON.stringify(units)}`).toBeDefined();
      // 50 EUR × 100 = 5000 hecto deposit
      expect(forex!.amount).toBe(50);
      expect(forex!.currency).toBe('EUR');
      // 50 × 1.10 = 55 USD (exact: 5500 hecto / 100 = 55)
      expect(forex!.forexAmount).toBe(55);
      expect(forex!.forexCurrency).toBe('USD');
      expect(forex!.exchangeRate).toBe(String(qvFxRate));
    });

    it('cross-currency DIVIDEND auto-fills forex_currency from security.currency when fxCurrencyCode is omitted', async () => {
      const { app, pid, accts, usdSecId } = await makeDividendFixture();
      ...
      const r = await request(app).post(`/api/p/${pid}/transactions`).send({
        ...
        accountId: accts.depositA,
        fxRate: 1.05,
        // fxCurrencyCode deliberately omitted — server must derive from securityId.currency
      });
      expect(r.status, `expected 201, got ${r.status} ${JSON.stringify(r.body)}`).toBe(201);

      const det = await request(app).get(`/api/p/${pid}/transactions/${r.body.uuid}`);
      const units = det.body.units as Array<{ type: string; forexCurrency: string | null }>;
      const forex = units.find((u) => u.type === 'FOREX');
      expect(forex).toBeDefined();
      expect(forex!.forexCurrency).toBe('USD');
    });

    it('same-currency DIVIDEND (EUR security on EUR deposit) requires no fxRate', async () => {
      const { app, pid, accts } = await makeDividendFixture();
      const eurSecId = randomUUID();
      const h = acquirePortfolioDb(pid);
      try {
        h.sqlite.prepare(
          `INSERT INTO security (_id, uuid, name, currency, updatedAt)
           VALUES (341, ?, 'EUR-DIV-SEC', 'EUR', '2026-01-01T00:00:00Z')`,
        ).run(eurSecId);
      } finally {
        releasePortfolioDb(pid);
      }
      ...
      // No FOREX unit — same-currency dividends are not cross-currency.
      const xid = r.body.uuid as string;
      const det = await request(app).get(`/api/p/${pid}/transactions/${xid}`);
      const units = det.body.units as Array<{ type: string }>;
      expect(units.find((u) => u.type === 'FOREX')).toBeUndefined();
    });
  });
