...
describe('POST /csv-import/trades/parse boundary hardening (BUG-46)', () => {
  ...
  // Defence-in-depth: non-browser clients can bypass the client-side
  // `sniffLikelyTradeCsv`; the server now rejects empty + binary buffers
  // before persisting the temp file.
  it('binary content (null byte) returns 400 INVALID_FILE_FORMAT', async () => {
    loadSettings();
    recoverFromInterruptedSwap();
    const app = createApp();
    const pid = await freshPortfolio(app, 'CSV-HARD-BIN');

    // Buffer.concat keeps the null byte out of the source literal.
  ...
  it('empty file returns 400 INVALID_FILE_FORMAT', async () => {
    loadSettings();
    recoverFromInterruptedSwap();
    const app = createApp();
    const pid = await freshPortfolio(app, 'CSV-HARD-EMPTY');
  ...
  it('price flow: binary content returns 400 INVALID_FILE_FORMAT', async () => {
    loadSettings();
    recoverFromInterruptedSwap();
    const app = createApp();
    const pid = await freshPortfolio(app, 'CSV-HARD-PRICE-BIN');

    const binaryHead = Buffer.concat([Buffer.from([0, 0]), Buffer.from('binary')]);
    const res = await request(app)
      .post(`/api/p/${pid}/csv-import/prices/parse`)
      .attach('file', binaryHead, {
        filename: 'bad.csv',
        contentType: 'text/csv',
      });
