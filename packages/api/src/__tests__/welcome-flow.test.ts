...
describe('welcome flow end-to-end', () => {
  ...
  it('export/import produces a real portfolio with a new UUID', async () => {
    ...
    // Upload as multipart. BUG-173: registry-name guard now runs on .db
    // restore symmetrically with PP-XML import, so the re-import must use
    // a non-colliding override name.
    const r2 = await request(app).post('/api/portfolios')
      .field('name', 'Source Restored')
