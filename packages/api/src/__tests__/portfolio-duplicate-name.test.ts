...
describe('POST /portfolios duplicate-name guard (BUG-05)', () => {
  it('second fresh portfolio with the same name returns 409 DUPLICATE_NAME', async () => {
    ...
    // BUG-PRE14-04: 409 echoes the actual conflicting registry name (preserved
    // case from the existing entry). Client uses this verbatim in the inline
    // alert instead of the user-supplied attempted name.
    expect(second.body.name).toBe('Alpha');
  });

  it('case-insensitive collision echoes preserved-case registry name (BUG-PRE14-04)', async () => {
    loadSettings();
    recoverFromInterruptedSwap();
    const app = createApp();

    const first = await request(app).post('/api/portfolios').send(freshBody('MixedCase Folio'));
    expect(first.status).toBe(201);

    const dup = await request(app).post('/api/portfolios').send(freshBody('mixedcase folio'));
    expect(dup.status).toBe(409);
    expect(dup.body.error).toBe('DUPLICATE_NAME');
    expect(dup.body.name).toBe('MixedCase Folio');
  });
