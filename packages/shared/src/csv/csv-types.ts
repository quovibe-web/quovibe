...
export const tradeColumnFields = [
  ...
  // Per-row account routing (BUG-126). When mapped, the cell value is an
  // account NAME (not UUID); the server resolves names via a batched
  // lookup. Blank cells fall back to the wizard's top-panel default.
  // `account` and `offsetAccount` resolve against deposit accounts
  // (type='account'); `securitiesAccount` and `offsetSecuritiesAccount`
  // against portfolio accounts (type='portfolio'). See
  // `.claude/rules/csv-import.md > Per-row account routing (BUG-126)`.
...
export const csvErrorCodes = [
  ...
  // INVALID_ACCOUNT_NAME: cell present, 0 matches in target account-type.
  // AMBIGUOUS_ACCOUNT_NAME: cell present, ≥2 matches in target type.
  // WRONG_ACCOUNT_TYPE: resolves uniquely but to wrong type.
  // MISSING_ACCOUNT: required-when cell blank (transfers).
  'INVALID_ACCOUNT_NAME', 'AMBIGUOUS_ACCOUNT_NAME',
  'WRONG_ACCOUNT_TYPE', 'MISSING_ACCOUNT',
] as const;
...
export interface RowError {
  ...
  code: CsvErrorCode;
  message: string;
  count?: number; // populated by AMBIGUOUS_ACCOUNT_NAME — number of matching accounts
}
...
export interface NormalizedTradeRow {
  ...
  // Per-row account name strings extracted from the CSV cells.
  // Resolved server-side to UUIDs by `resolveAccountNames` in
  // `csv-import.service.ts`. Trim-applied at parse time; '' becomes
  // undefined.
  accountName?: string;
  securitiesAccountName?: string;
  offsetAccountName?: string;
  offsetSecuritiesAccountName?: string;
  // Per-row resolved account UUIDs. Populated by the service layer after
  // `resolveAccountNames` runs against the four name fields above; consumed
  // by `csv-trade-mapper.ts` to route `xact.account` and the cross-entry
  // legs per row instead of always using the file-level
  // `ctx.portfolioId` / `ctx.depositAccountId`. Each field falls back to
  // the file-level default when absent (the mapper's per-type matrix
  // documents which fallback applies where).
  accountUuids?: {
    account?: string;
    securitiesAccount?: string;
    offsetAccount?: string;
    offsetSecuritiesAccount?: string;
  };
}
