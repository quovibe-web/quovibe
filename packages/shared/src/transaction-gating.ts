...
// `security`, which `@quovibe/shared` cannot perform.
//
// Membership rationale per type:
//   BUY / SELL                — cash leg currency vs security currency
//   DIVIDEND                  — same as BUY/SELL: dividends paid into a deposit
//                                whose currency differs from the security's
//                                currency need an fxRate so the engine can
//                                book the security-ccy payout against the
//                                deposit-ccy cash flow with no silent drift
//   TRANSFER_BETWEEN_ACCOUNTS — source vs destination deposit currency
