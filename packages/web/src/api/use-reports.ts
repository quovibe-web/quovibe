...
type ScopedApi = ReturnType<typeof useScopedApi>;

/**
 * Single source of truth for the payments-breakdown query — used by both the
 * `usePaymentsBreakdown` consumer and by `Payments.tsx`'s on-hover prefetch.
 * Drift between the two sites silently turns the prefetch into a cache miss.
 */
export function paymentsBreakdownQueryOptions(
  api: ScopedApi,
  bucket: string,
  ...
  return {
    ...
    queryFn: () =>
      api.fetch<PaymentBreakdownResponse>(
...
export function usePaymentsBreakdown(
  ...
  return useQuery({
    ...
    queryFn: () => {
      ...
      return paymentsBreakdownQueryOptions(api, bucket, type, groupBy, periodStart, periodEnd).queryFn();
    },
    enabled: !!bucket,
    staleTime: 5 * 60 * 1000,
    placeholderData: keepPreviousData,
