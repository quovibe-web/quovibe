...
import { getDateLocale, formatCurrency, formatNumber } from '@/lib/formatters';
import {
...
export function TransactionForm({
  ...
  // inline errors set by the schema-rebuild trigger() below before the user
  // ever blurs the field. Skip after blur — RHF's native path takes over.
  useEffect(() => {
  ...
  const feesNum = parseFloat(watchedFees || '0') || 0;
  const taxesNum = parseFloat(watchedTaxes || '0') || 0;
  const fx = computeFxAmounts({
    isCrossCurrency, fxRate: fxRateVal, grossSecurity,
    feesFx: feesFxNum, taxesFx: taxesFxNum,
    feesDeposit: feesNum,
    taxesDeposit: taxesNum,
  });

  // Display-only mirrors for fee/tax placeholders. Wire stays additive
  // (`fees + feesFx/fxRate`, see csv-import.md > Foreign-currency fees and taxes).
  const canMirror = isCrossCurrency && fxRateVal > 0;
  const mirrorPlaceholder = (thisVal: string, otherNum: number, convert: number) =>
    canMirror && !thisVal && otherNum > 0
      ? `≈ ${formatNumber(otherNum * convert, { minimumFractionDigits: 2, maximumFractionDigits: 4 })}`
      : undefined;
  const feesFxPlaceholder  = mirrorPlaceholder(watchedFeesFx,  feesNum,    fxRateVal);
  const feesPlaceholder    = mirrorPlaceholder(watchedFees,    feesFxNum,  1 / fxRateVal);
  const taxesFxPlaceholder = mirrorPlaceholder(watchedTaxesFx, taxesNum,   fxRateVal);
  const taxesPlaceholder   = mirrorPlaceholder(watchedTaxes,   taxesFxNum, 1 / fxRateVal);
          ...
          <div className="rounded-md border bg-muted/30 p-3 space-y-2">
            {BUY_SELL_TYPES.has(type) && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{formatCurrency(grossSecurity, dstCurrency, { showCurrencyCode: true })}</span>
              </div>
            )}
            ...
            {BUY_SELL_TYPES.has(type) && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">{formatCurrency(fx.grossDeposit, srcCurrency, { showCurrencyCode: true })}</span>
              </div>
            )}
            ...
            {type === TransactionType.TRANSFER_BETWEEN_ACCOUNTS && (
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium">
                  {formatCurrency((parseFloat(watchedAmount || '0') || 0) * fxRateVal, dstCurrency, { showCurrencyCode: true })}
                </span>
              </div>
            )}
              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <NumericField
                    ...
                    placeholder={taxesFxPlaceholder}
                  />
                  <NumericField
                    form={form}
                    name="taxes"
...
function NumericField({ form, name, label, fieldId, placeholder = '—' }: NumericFieldProps) {
  return (
    <FormField
      control={form.control}
      name={name}
