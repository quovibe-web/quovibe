...
export function EditSellDialog({ open, onOpenChange, transaction }: Props) {
  ...
  // SELL ppxml2db convention: xact.amount = gross - fees - taxes (deposit ccy).
  // For cross-currency SELLs the FOREX unit carries the security-ccy gross
      ...
      <Sheet open={open} onOpenChange={guardedOpenChange}>
        <SheetContent side="right" className="w-full sm:max-w-lg p-0 flex flex-col" showCloseButton>
          <SheetHeader className="px-6 pt-6 pb-2 shrink-0">
            ...
            <SheetDescription className="sr-only">{t('editTitles.sell')}</SheetDescription>
          </SheetHeader>
