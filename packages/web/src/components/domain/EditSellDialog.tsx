...
export function EditSellDialog({ open, onOpenChange, transaction }: Props) {
  ...
  useEffect(() => {
    if (!txDetail) return;
    const fxData = extractFxFromUnits(txDetail.units);
    setFxRate(fxData.fxRate);
    setFeesFx(fxData.feesFx);
    setTaxesFx(fxData.taxesFx);
  }, [txDetail]);
  ...
  useEffect(() => {
    if (isCrossCurrency && fxRateData?.rate && !fxRate) {
      setFxRate(fxRateData.rate);
      setIsDirty(true);
    }
  }, [fxRateData?.rate, isCrossCurrency]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const creditNote = isCrossCurrency
