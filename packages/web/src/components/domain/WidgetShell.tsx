...
import { GripHorizontal, MoreHorizontal, Clock, X, Pencil } from 'lucide-react';
        ...
        <CardHeader className={cn(
          ...
          <div className="flex-1 min-w-0 flex flex-col gap-0.5">
            {editing ? (
              ...
              <span
                className={cn(
                  'inline-flex items-center gap-1 text-sm font-medium cursor-pointer transition-colors leading-tight',
                  hidden
                    ? 'text-muted-foreground/60 italic line-through hover:text-muted-foreground'
              ...
              >
                <span>{title}</span>
                {!hidden && <Pencil className="h-3 w-3 opacity-60 shrink-0" />}
              </span>
            )}
              ...
              <DropdownMenuContent align="end">
                ...
                {onToggleHidden && (
                  <DropdownMenuItem onSelect={onToggleHidden}>
                    {t(hidden ? 'showWidget' : 'hideWidget')}
                  </DropdownMenuItem>
                )}
                <DropdownMenuSeparator />
