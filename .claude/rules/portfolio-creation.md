...
| `CreatePortfolioSource` | N securities accounts after creation | Returns summary? | Notes |
|---|---|---|---|
| `fresh` | 1 + N≥0 user-supplied extras | **No** | Seeded by `createFreshImpl` from the M3 dialog payload. Registry-name guard runs (409 `DUPLICATE_NAME` on collision with any real or demo entry). |
| `demo` | 2 (Interactive Brokers + Scalable Capital) | **No** | Comes from the seeded `data/demo.db` template. Unchanged. |
| `import-pp-xml` | N≥1 from PP's wizard | **Yes** | summary populated by `computeImportSummary` in `portfolio-manager.ts`. PP enforces the N≥1 invariant. **Registry-name guard runs (BUG-92): a re-import whose derived name collides with any existing entry returns 409 `DUPLICATE_NAME`, and the ImportHub dialog lets the user rename and retry.** |
| `import-quovibe-db` | N from the source DB (N=0 possible for backups taken pre-fix) | **Yes** | summary populated by `computeImportSummary` in `portfolio-manager.ts`. `PortfolioLayout` setup-redirect catches the N=0 case. **Registry-name guard runs (BUG-173)**: a re-import whose effective name (override or source's `vf_portfolio_meta.name`) collides with any existing entry returns 409 `DUPLICATE_NAME`, and the ImportHub `.db` source card carries an optional rename input that becomes the override. The pre-fix posture ("legitimate overwrite flow, bypass intentional") was fictional — the bypass produced silent duplicate registry entries indistinguishable in the switcher rather than any kind of overwrite. |

`demo` is not touched by the BUG-54 fix. `import-pp-xml` gained the
registry-name guard under BUG-92 (see `createImportedPpxmlImpl` comment) and
`import-quovibe-db` gained the same guard under BUG-173 (see
`createImportedQuovibeDbImpl`). The UX shape is symmetric across all three
real-portfolio sources: fresh/PP-XML/`.db` all surface 409 on collision and
the user retypes a non-colliding name.

## Duplicate-name invariant (BUG-05 / BUG-102 / BUG-92 / BUG-173)
...
`renamePortfolio`, `createImportedPpxmlImpl`, and `createImportedQuovibeDbImpl`.
Comparison is trimmed + case-insensitive and runs against **all** entries
(real and demo) — two rows rendering as the same label in the switcher are
indistinguishable regardless of `kind`. `selfId` lets `renamePortfolio` skip
its own entry so a same-name PATCH is a no-op 200 instead of a self-collision
409. Client surfaces of the 409 translate the raw `DUPLICATE_NAME` code via
`errors.portfolio.duplicateName` (BUG-70); see `NewPortfolioDialog`,
`RenamePortfolioDialog`, `ImportHub` (covers both PP-XML and `.db` flows).

`createImportedQuovibeDbImpl` accepts an optional `name` parameter (BUG-173).
When present and non-blank (after trim), it becomes the effective registry
name; otherwise the source's `vf_portfolio_meta.name` is used. Either way the
guard runs against the effective name. The override does NOT rewrite the
inner DB's `vf_portfolio_meta.name` — that intentional drift means the user's
rename only retitles the registry/switcher entry while the embedded backup
metadata is preserved as-is. Future contributors must NOT "normalize" by
syncing inner meta on override; the registry name is the user-facing label,
the inner meta is the backup's archival self-description. Known second-order
effect: if the sidecar registry is later lost and rebuilt via
`rebuildRegistryFromDbs`, the rescued entry reverts to the inner meta's
`name` — possibly re-creating a collision with a sibling portfolio's
non-overridden name. Acceptable today (rescue is a recovery path, not a
hot loop) but a candidate for a future "rescue with collision-resolution"
pass if the orphan-rebuild flow gains usage.
