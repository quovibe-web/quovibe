...
### Create mutations — pre-populate the parent list cache (BUG-166)

Bare `invalidateQueries({queryKey: ['portfolios', pid, entity]})` in a create
mutation's `onSuccess` is sufficient ONLY when the creating component does
not navigate away inside its own call-site `onSuccess`. The dashboard route
mounts `Dashboard.tsx` at TWO distinct paths (`/dashboard` and
`/dashboard/:dashboardId`); navigating from the first to the second
unmounts the first observer before the invalidate-driven refetch lands,
and the new mount sees the pre-create cache snapshot. The QA pass that
filed BUG-166 saw exactly this: tab strip stayed on "Overview" until F5
even though the server held both rows. The class fix is the
`useEntityMutations` factory in
`packages/web/src/api/use-entity-mutations.ts`, which:

1. Calls `setQueryData<TItem[]>(listKey, prev => [...prev, created])` in
   `onSuccess` BEFORE invalidating — so the post-navigate remount reads a
   cache that already contains the new entity, even if the in-flight
   refetch was cancelled by the unmount.
2. Returns the `invalidateQueries` promise (rather than fire-and-forget) so
   any caller-side `onSuccess` (typically a `navigate(...)`) runs after the
   refetch settles, guaranteeing eventual consistency on the rare path
   where the optimistic shape diverges from the server's view.

New entity collections (REST shape: list at `/api/<entity>`, item at
`/api/<entity>/:id`, JSON bodies, parent-prefix query keys) MUST go through
`useEntityMutations`. Hooks with bespoke needs (file uploads, cross-key
fan-out invalidation, special error mapping — `useCreatePortfolio`,
`useDeletePortfolio`, `useCreateAccount`'s 6-table fan-out) keep their own
implementation. The factory's create/update/delete options builders
(`buildCreateMutationOptions` / `buildUpdateMutationOptions` /
`buildDeleteMutationOptions`) are exported separately so a bespoke hook can
still adopt the cache-reconciliation posture without giving up its bespoke
mutationFn.
...
1. `useForm<Shape>({ resolver: zodResolver(schema), mode: 'onBlur',
   ...
   per-field validation as the user moves between fields and keeps mount
   silent (no red hints on first paint). **Load-bearing caveat:** RHF's
   `reValidateMode: 'onChange'` only kicks in AFTER submit — not after
   `form.trigger()` and not after a blur-driven validation pass. Without
   extra wiring, an inline error that surfaced via blur or via a
   schema-rebuild `form.trigger()` will persist while the user fixes the
...
- Dim-fraction styling (`muted-fraction` CSS class on NumberFlow) applies **only** to currency values via `<CurrencyDisplay>`. Percentages, ratios (Sharpe), and shares render full-weight everywhere — including the dashboard hero, widgets, and `ChartSummaryBar`. Audit-grade convention (Bloomberg, IBKR, Directa, Wealthfolio) treats percent precision as the signal, not noise. Do not extend the class to any other primitive.
