...
### Per-row override via CSV import (BUG-126)

The CSV import path supports per-row account routing via four optional
columns (`account`, `securitiesAccount`, `offsetAccount`,
`offsetSecuritiesAccount`). Resolution + per-type matrix details live
in `.claude/rules/csv-import.md > Per-row account routing (BUG-126)`.
Per row, `xact.account` becomes the resolved UUID (or the file-level
fallback when the slot is blank). The double-entry shape (xact #1
securities-side + xact #2 cash-side, plus `xact_cross_entry` wiring)
is unchanged — only the `account` value per row varies.
...
| DIVIDEND                  | **req**      | —        | > 0      | deposit *or* portfolio (auto-route) | —    | cash.currency ≠ security.currency  |
...
- **Route layer** (`packages/api/src/routes/transactions.ts`):
  ...
  - `enforceCrossCurrencyFxRate` (run after the type guard) requires
    ...
    - BUY/SELL cash-side (resolved to `crossAccountId` or
      ...
      (BUG-112), OR
    - DIVIDEND cash-side (the resolved deposit — directly when
      `accountId` is a deposit, via `referenceAccount` when it is a
      portfolio) currency differs from `security.currency`. The
      service-layer `buildUnits` DIVIDEND branch emits the matching
      FOREX `xact_unit` row when `fxRate` is supplied so JSON and CSV
      ingestion paths persist byte-identical FOREX shapes.
